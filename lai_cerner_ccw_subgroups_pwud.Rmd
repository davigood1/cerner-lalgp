...
predvars_list_pwud <- c(
  ...
  "oud", "cud", "mud", "idu",
  "MOUD_Methadone", "MOUD_Buprenorphine",
  ...
  future_map_dfr(1:500, ~{
    safely_run <- safely(function() {
      resample_index <- sample(nrow(
        df.ccw %>% 
          filter(pwud == "Yes", !!sym(subgroup_name) == 1)), 
        replace = TRUE)
      boot_sample <- df.ccw %>% 
        filter(pwud == "Yes", !!sym(subgroup_name) == 1) %>% 
...
```{r}
df.ccw %>% filter(pwud == "Yes", antibiotics == "Vancomycin" | laLGP == 1)
...
boot_results_antibiotics <- future_map(antibiotics, ~{
  ...
  future_map_dfr(1:500, ~{
    resample_index <- sample(nrow(
      df.ccw %>% 
        filter(pwud == "Yes", antibiotics == antibiotic_name | laLGP == 1)), 
      replace = TRUE)
    boot_sample <- df.ccw %>% 
      filter(pwud == "Yes", antibiotics == antibiotic_name | laLGP == 1) %>% 
      slice(resample_index)
...
results_table_combined <- bind_rows(
  results_table_subgroups %>% mutate(Group = "Subgroup"),
  results_table_antibiotics %>% mutate(Group = "Antibiotic")
)
...
gtsave(final_table, "laLGP_Effect_Results_pwud_subgroups_stanrdized.docx")
```