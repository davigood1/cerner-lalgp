```{r}
library(pacman)
p_load(tidyverse, tidylog, survival, survminer, boot, RColorBrewer, gtsummary, future, furrr, gt)
p_load_gh("Genentech/survivalCCW")

# Define the JAMA color palette (e.g., 2 colors for the strata)
jama_colors <- brewer.pal(2, "Set1")

extract_results <- function(group_name, boot_results) {
  boot_results %>%
    bind_rows() %>%
    summarise(
      HR = mean(hr.clone, na.rm = TRUE),
      `95% CI (HR)` = paste0(
        quantile(hr.clone, 0.025, na.rm = TRUE), " - ", 
        quantile(hr.clone, 0.975, na.rm = TRUE)
      ),
      RMST_Diff_90d = mean(rmst_diff.rmean, na.rm = TRUE),
      `95% CI (RMST)` = paste0(
        quantile(rmst_diff.rmean, 0.025, na.rm = TRUE), " - ", 
        quantile(rmst_diff.rmean, 0.975, na.rm = TRUE)
      )
    ) %>%
...
#Create dummy variables for categorical indicators
p_load(fastDummies)
df.ccw <- df.ccw %>%
  mutate(across(c(gender, race, ethnicity, insurance, MOUD, zip_code, bed_size), 
                ~ gsub(" ", "_", as.character(.)))) %>%
  mutate(across(c(zip_code, bed_size), 
                ~ gsub("-", "", as.character(.)))) %>%
  mutate(across(c(zip_code, bed_size), 
                ~ gsub(",", "", as.character(.)))) %>%
  mutate(across(c(bed_size), 
                ~ gsub("<", "", as.character(.)))) %>%
  dummy_cols(
    select_columns = c("gender", "race", "ethnicity", "insurance", "MOUD", "zip_code", "bed_size", "discharge_disposition"), 
    remove_most_frequent_dummy = TRUE,   # Avoids multicollinearity
    remove_selected_columns = TRUE,
    ignore_na = T)  # Removes original categorical columns

df.ccw <- df.ccw %>%
  mutate(across(starts_with("zip_code"), ~ replace_na(., 0)))
```
...
gtsave(final_table, "laLGP_Effect_Results_pwud_main_standardized.docx")
...
# Run bootstrapping with future_map_dfr in a safely wrapped function
boot_out.non.pwud <- future_map_dfr(1:500, ~{
  ...
  if (is.null(result$error)) {
    ...
    message("Error in iteration: ", result$error)
    return(NULL)  # Ensures the function continues running even if an error occurs
  }
}, .progress = TRUE, .options = furrr_options(seed = 1))
...
# Create gt table
final_table <- boot_results_non_pwud %>%
  mutate(
    `HR (95% CI)` = paste0(round(hr_mean, 2), 
                           " (", round(hr_lower, 2), " - ", round(hr_upper, 2), ")"),
    `RMST Diff 90d (95% CI)` = paste0(round(rmst_mean, 2), 
                                      " (", round(rmst_lower, 2), " - ", round(rmst_upper, 2), ")")
  ) %>%
  select(`HR (95% CI)`, `RMST Diff 90d (95% CI)`) %>%
  gt() %>%
  tab_header(title = "Effect of laLGP on Composite Outcomes in Non-PWUD") 

final_table

# Save as Word document
gtsave(final_table, "laLGP_Effect_NonPWUD_windsorize.docx")
```
